...
# ----- 005: /cancel canonical + alias deprecation -------------------------
...
class TestAliasDeprecation:
    """005 US2: /done + plain-text done/stop/finish keep working with WARNING."""

    async def test_slash_done_emits_alias_deprecation_audit(
        self,
        tmp_path,
        conn: sqlite3.Connection,
        client: TelegramClient,
        ...
        def _has(alias: str, session: str) -> bool:
            return (alias, session) in warned

        def _record(alias: str, session: str) -> None:
            warned.add((alias, session))
        ...
        ctx = rt_dispatcher.DispatchContext(
            ...
            is_operator_stop_in_flight=lambda s: False,
            worker_pid_for_session=lambda s: 99999,
            has_alias_deprecation_warned=_has,
            record_alias_deprecation_warned=_record,
        )
        ...
        # Alias deprecation set was populated.
        assert ("/done", sid) in warned
        ...
        assert len(deprec) == 1
        assert deprec[0]["alias_token"] == "/done"
        assert deprec[0]["canonical"] == "cancel"
        assert deprec[0]["session_id"] == sid

    async def test_slash_done_idempotent_per_session(
        self,
        tmp_path,
        conn: sqlite3.Connection,
        client: TelegramClient,
        ...
        ctx = rt_dispatcher.DispatchContext(
            ...
            is_operator_stop_in_flight=lambda s: False,
            worker_pid_for_session=lambda s: 99999,
            has_alias_deprecation_warned=lambda a, s: (a, s) in warned,
            record_alias_deprecation_warned=lambda a, s: warned.add((a, s)),
        )
        ...
        # Two consecutive /done invocations on the same (alias, session) pair.
        for i in range(2):
            msg = _slash_message(
                "/done",
                sender_id=99001,
        ...
        deprec = [e for e in events if e.get("event_type") == "alias_deprecation_used"]
        # Only ONE WARNING despite two /done invocations on the same session.
        assert len(deprec) == 1

    async def test_slash_done_main_chat_uses_done_reason(
        self,
        tmp_path,
        conn: sqlite3.Connection,
        client: TelegramClient,
        ...
        """R5: /done in main chat MUST audit reason=main_chat_done (NOT main_chat_cancel)."""
        from remotask.core import logging as rt_logging
        ...
        msg = _slash_message(
            "/done", sender_id=99001, chat_id=fake_tg.chat_id, message_id=53
        )
        await rt_dispatcher.dispatch(msg, ctx)
        ...
        rejected = [e for e in events if e.get("event_type") == "slash_command_rejected"]
        assert len(rejected) == 1
        assert rejected[0]["reason"] == "main_chat_done"
        assert rejected[0]["command"] == "done"

    async def test_plaintext_alias_emits_deprecation_warning(
        self,
        tmp_path,
        conn: sqlite3.Connection,
        client: TelegramClient,
        ...
        msg = {
            **_message("stop", sender_id=99001, chat_id=fake_tg.chat_id, message_id=54),
            "message_thread_id": topic_id,
        }
        await rt_dispatcher.dispatch(msg, ctx)

        assert ("stop", sid) in warned
        ...
        deprec = [e for e in events if e.get("event_type") == "alias_deprecation_used"]
        assert len(deprec) == 1
        assert deprec[0]["alias_token"] == "stop"
        assert deprec[0]["canonical"] == "cancel"
