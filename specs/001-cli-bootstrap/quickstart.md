# Quickstart: remote-task 첫 설치

**Feature**: 001-cli-bootstrap
**Date**: 2026-05-01
**대상 사용자**: 처음 remote-task를 설치하는 1인 사용자
**소요 시간 목표**: 5분 (SC-001)
...
```bash
remote-task --version            # → 버전 문자열 한 줄
remote-task --help               # → 8개 서브커맨드 목록
remote-task daemon --help        # → run-foreground/start/stop/status/logs
```
...
```
✓ Created /Users/samuel/.config/remote-task/config.toml (mode 0600)
✓ Created /Users/samuel/.local/share/remote-task/state.db (schema v1)
✓ Created /Users/samuel/.local/share/remote-task/logs/
✓ Generated daemon.auth_token (saved to config.toml)

다음 단계:
  remote-task config set telegram.bot_token <YOUR_TOKEN>
  remote-task install
```
...
```bash
ls -la ~/.config/remote-task/
ls -la ~/.local/share/remote-task/
sqlite3 ~/.local/share/remote-task/state.db ".tables"     # 5개 테이블
stat -f "%Lp" ~/.config/remote-task/config.toml           # → 600
```

**멱등성 확인:**

```bash
remote-task init                 # → "Already initialized; nothing to do"
```
...
# 토큰 마스킹 확인
remote-task config get daemon.auth_token
# → ****abcd

# 원문 노출 (필요 시만)
remote-task config get daemon.auth_token --reveal

# 잘못된 키 거부
remote-task config set foo.bar 1
# → Error: 'foo.bar'는 정의되지 않은 키입니다.
#   사용 가능한 키: agent.max_concurrent, agent.worktree_root, ...

# 잘못된 형식 거부
remote-task config set agent.max_concurrent abc
...
```bash
echo "99999" > ~/.local/share/remote-task/daemon.pid    # 가짜 PID
remote-task daemon status
...
```bash
remote-task install
# → ✓ Wrote ~/Library/LaunchAgents/kr.mission-driven.remote-task.plist
...
```bash
launchctl list | grep remote-task
# → 23456    0    kr.mission-driven.remote-task
...
- [ ] `pytest --cov=remote_task tests/` 커버리지 70% 이상
- [ ] `remote-task --help` 응답 1초 이내
- [ ] `remote-task init` 응답 3초 이내
- [ ] `remote-task daemon stop` 5초 이내 종료
...
### "command not found: remote-task"
- `uv tool install .` 후 `~/.local/bin`이 PATH에 있는지 확인
- 또는 `uv run remote-task ...`로 실행

### "claude not found in PATH" (install 단계)
- `claude` CLI 설치 경로를 PATH에 추가
- 또는 `remote-task install`을 호출할 때 사용자 셸이 그 경로를 포함하는지 확인

### launchd 데몬이 부팅 후 실행되지 않음
- `tail -f ~/.local/share/remote-task/logs/launchd.err.log`
- PATH·HOME 등 환경변수 누락 가능성 → `remote-task uninstall && remote-task install`로 재등록
