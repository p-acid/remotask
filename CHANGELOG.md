...
기능 단위 history. append-only — 신규 entry는 위(최신순). 각 섹션은 5~15줄
이내로 짧게: motivation → 핵심 결과 → PR / ARD 참조. 디테일이 필요하면
ARD entry나 코드를 본다.
...
## 007 — Agent SDK Integration (placeholder worker → real claude-agent-sdk)

**PR**: [#6](https://github.com/p-acid/remotask/pull/6) · **ARD**: D22 ·
**Branch**: `007-agent-sdk-integration` (머지 후 삭제)
...
**Branch**: `006-remove-termination-aliases` (머지 후 삭제)
...
§III 완화 v1.0.0 → v1.1.0), D20 · **Branch**: `005-dm-channel`
...
## 002 — Telegram trigger (listener · dispatcher · topic · audit · worker scaffolding)
...
## Process — 2026-05-03 process overhaul (예정)
